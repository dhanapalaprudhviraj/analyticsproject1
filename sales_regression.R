...
plot(y=df3$Y, x=X,xlab='Area in sqft', ylab='Sales Amount', type='p', main='Plot of Area Vs Sales',ylim=c(0,max(df1$Y)),col='red',pch=15)
abline(lm(df1$Y ~ df1$X),lty=3,lwd=4,col='green')
...
coef(fit1)[2]
cbind(df1,fitted(fit1),fitted(fit1)-df1$Y,residuals(fit1))
# Predictions
...
plot(x=df1$X, y=residuals(fit1)) # Linearity
abline(h=0)
plot(residuals(fit1))
